...
async function handleMountainSharesPurchase(session) {
  ...
  try {
    ...
    const purchaseData = extractPurchaseData(session);
    
    const results = await Promise.allSettled([
      processSettlementReserveDeposit(purchaseData, transactionId),
      processH4HTreasuryDistribution(purchaseData, transactionId),
...
function extractPurchaseData(session) {
  ...
  const platformFee = tokensPurchase * platformFeeRate;
  const subtotal = tokensPurchase + platformFee;
...
async function processSettlementReserveDeposit(purchaseData, transactionId) {
  ...
  console.log('💰 Processing Settlement Reserve Deposit:', {
    transactionId: transactionId,
    amount: `$${purchaseData.netReceived} USDC`,
  ...
  return {
    ...
    amount: purchaseData.netReceived,
    contractAddress: settlementReserve,
    status: 'logged'
  };
}

async function processH4HTreasuryDistribution(purchaseData, transactionId) {
  const distribution = {
    nonprofit: {
      address: process.env.H4H_NONPROFIT_ADDRESS,
      amount: (purchaseData.platformFee * 30) / 100,
      percentage: 30
    },
    communityPrograms: {
      address: process.env.H4H_COMMUNITY_PROGRAMS_ADDRESS,
      amount: (purchaseData.platformFee * 15) / 100,
      percentage: 15
    },
    treasury: {
      address: process.env.H4H_TREASURY_MOUNTAINSHARES_ADDRESS,
      amount: (purchaseData.platformFee * 30) / 100,
      percentage: 30
    },
    governance: {
      address: process.env.H4H_GOVERNANCE_ADDRESS,
      amount: (purchaseData.platformFee * 10) / 100,
      percentage: 10
    },
    development: {
      address: process.env.DEVELOPMENT_ADDRESS,
      amount: (purchaseData.platformFee * 15) / 100,
      percentage: 15
    }
  };
  
  console.log('📊 Processing H4H Treasury Distribution:', {
    transactionId: transactionId,
    totalPlatformFee: `$${purchaseData.platformFee}`,
    distributions: Object.entries(distribution).map(([key, value]) => ({
      component: key,
      amount: `$${value.amount}`,
  ...
  return {
    action: 'h4h_treasury_distribution',
    totalAmount: purchaseData.platformFee,
